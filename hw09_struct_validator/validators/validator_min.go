...
func newValidatorMin(args string, value interface{}) (Validator, error) {
	...
	builder.setFunctionsForKind(
		...
		func(pArgs parsedArgs, value reflect.Value) error {
			...
			if value.Int() < threshold {
				return fmt.Errorf("%d is less than %d", value.Int(), threshold)
			}
			return nil
		},
	...
	builder.setFunctionsForKind(
		...
		func(pArgs parsedArgs, value reflect.Value) error {
			...
			if value.Uint() < threshold {
				return fmt.Errorf("%d is less than %d", value.Uint(), threshold)
			}
			return nil
		},
	...
	builder.setFunctionsForKind(
		...
		func(pArgs parsedArgs, value reflect.Value) error {
			...
			if value.Float() < threshold {
				return fmt.Errorf("%f is less than %f", value.Float(), threshold)
			}
			return nil
		},
	...
	builder.setFunctionsForKind(
		...
		func(pArgs parsedArgs, value reflect.Value) error {
			...
			if int64(l) < threshold {
				return fmt.Errorf("%d is less than %d", l, threshold)
			}
			return nil
		},
		reflect.String,
